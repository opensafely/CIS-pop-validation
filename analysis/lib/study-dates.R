...
study_dates <- tibble::lst(
  start_date = "2020-04-26", # Sunday 26 April 2020, one day before the first date of ONS-CIS
  end_date = "2022-06-26", # Sunday 26 June 2022, should be updated when rerun
)
...
## This makes it easier to run "look-back" periods, over 1, 14 and infinity days.
## Single-day estimates therefore fall on a sunday, which isn't the best day to use.
## So focus on fortnightly averages.


jsonlite::write_json(study_dates, path = here::here("analysis", "lib", "study-dates.json"), auto_unbox=TRUE, pretty =TRUE)
